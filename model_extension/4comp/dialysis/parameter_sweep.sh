#!/bin/bash
# EEE2+E2+Q2+K2 concentrations:  0.6 0.55 0.5 0.4 0.325 0.3 0.245 0.2 0.16 0.125 0.1 0.075 0.04 (decide on the sequence, change downstairs)
...
if [ "$FOURCOMP" = "EEE2E2Q2K2" ]; then
  ...
  restartdir="$USERVLP1"_"$USERVLP2"_"$USERVLP3"_"$USERVLP4"_c"$USERHIGHSALT"
  cp -a template/ $dir
  cp $restartdir/Restartc${USERHIGHSALT}.* $dir/ # restart name should match the name in the input script within template/
  cd $dir
    sed -i 's/USERHIGHSALT/'$USERHIGHSALT'/g' in.4comp.template
